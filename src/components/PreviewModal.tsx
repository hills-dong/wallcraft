...
export default function PreviewModal() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const success = await invoke<boolean>('apply_wallpaper', { photoUrl: fullUrl })
      if (success) {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {cropPreviews ? (
            <div>
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-200">
                <div style={{ position: 'relative', width: `${cropPreviews.containerW}px`, height: `${cropPreviews.containerH}px`, backgroundColor: '#111' }}>
                  {cropPreviews.screenPreviews.map(({ screen, index, left, top, width, height, bgPosX, bgPosY, bgW, bgH }) => (
                    <div
                      key={screen.id}
                      style={{
                        position: 'absolute',
                    ...
                    >
                      <div style={{ position: 'absolute', bottom: 6, left: 6, background: 'rgba(0,0,0,0.6)', color: 'white', fontSize: 11, padding: '3px 7px', borderRadius: 5, lineHeight: 1.5, backdropFilter: 'blur(6px)' }}>
                        Screen {index}
                        <span style={{ opacity: 0.7, marginLeft: 4 }}>{screen.width}x{screen.height}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-xl overflow-hidden border border-gray-200">
              <img src={previewPhoto.urls.regular} alt={previewPhoto.description || 'Preview'} className="w-full h-auto" />
            </div>
          )}
            ...
            <div className="space-y-2">
              {wallpaperStatuses.map((status) => (
                <div key={status.screenId} className="flex items-center gap-3 text-sm">
                  ...
                  <span className={`font-medium ${status.status === 'success' ? 'text-green-600' : status.status === 'error' ? 'text-red-600' : 'text-blue-600'}`}>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <span>Photo by</span>
            <a href={previewPhoto.user.links.html} target="_blank" rel="noopener noreferrer" className="text-[#007AFF] hover:underline font-medium">
              {previewPhoto.user.name}
            </a>
            <span>on</span>
            <a href="https://unsplash.com" target="_blank" rel="noopener noreferrer" className="text-[#007AFF] hover:underline font-medium">
              Unsplash
            </a>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 p-4 border-t border-gray-100">
          <button onClick={handleClose} disabled={isApplying} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-50">
            Close
          </button>
          <button
            onClick={handleApply}
